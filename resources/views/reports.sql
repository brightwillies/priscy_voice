-- Create temporary table for operational report
...
-- Create temporary table for executive report
CREATE TEMPORARY TABLE executive_report AS
SELECT
    r.region_name AS region,
    cat.category_name AS category,
    COUNT(DISTINCT o.order_id) AS total_orders,
    COUNT(DISTINCT cl.customer_id) AS unique_customers,
    SUM(CAST(od.sales AS DECIMAL(10,2))) AS total_sales,
    SUM(CAST(od.profit AS DECIMAL(10,2))) AS total_profit,
    (SUM(CAST(od.profit AS DECIMAL(10,2))) / NULLIF(SUM(CAST(od.sales AS DECIMAL(10,2))), 0) * 100 AS profit_margin_percentage,
    SUM(CAST(od.discount AS DECIMAL(10,2))) AS total_discount,
    COUNT(DISTINCT CASE WHEN ret.returned IS NOT NULL THEN o.order_id END) AS return_count,
    COUNT(DISTINCT CASE WHEN ret.returned IS NOT NULL THEN o.order_id END) * 100.0 / NULLIF(COUNT(DISTINCT o.order_id), 0) AS return_rate_percentage,
    SUM(CAST(od.quantity AS DECIMAL(10,2))) AS total_units_sold
FROM
    orders o
JOIN
...
JOIN
    category cat ON sub.category_id = cat.category_id
JOIN
    `customer location` cl ON o.cust_loc_id = cl.cust_loc_id
JOIN
    location loc ON cl.location_id = loc.location_id
JOIN
    region r ON loc.region_id = r.region_id
LEFT JOIN
    returns ret ON o.order_id = ret.order_id AND od.product_id = ret.product_id
GROUP BY
    r.region_name, cat.category_name
ORDER BY
